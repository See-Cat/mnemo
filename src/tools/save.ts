  ...
  server.registerTool(
    "memory_save",
    {
      ...
      inputSchema: {
        ...
        source: z
          ...
          .optional()
          .describe(
            "Source identifier, e.g., 'opencode', 'claude-code'. Defaults to 'unknown'."
          ),
      },
    },
    async ({ content, tags, source }) => {
      try {
        ...
        if (
          stats.count > COMPRESS_THRESHOLDS.maxNotes ||
          stats.totalSize > COMPRESS_THRESHOLDS.maxTotalSize
        ) {
